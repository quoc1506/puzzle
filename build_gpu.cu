...
int run_gpu_verify(int target_puzzle = 0, int target_device_id = 0) {
    ...
    for (size_t idx = 0; idx < NUM_SOLVED_PUZZLES; ++idx) {
        ...
        if (target_puzzle > 0 && sp.id != target_puzzle) continue;
        ...
        uint64_t half = chunk_size / 2;
        u256 start_k = 1;
        if (expected_k > half) {
            start_k = expected_k - half;
        }

        cudaMemset(d_found_flag, 0, sizeof(int));
        cudaMemset(d_found_offset, 0, sizeof(uint64_t));

        uint64_t sk0 = (uint64_t)start_k.low;
        uint64_t sk1 = (uint64_t)(start_k.low >> 64);
        uint64_t sk2 = (uint64_t)start_k.high;
        uint64_t sk3 = (uint64_t)(start_k.high >> 64);
        uint32_t cur_batches = (steps_per_launch + 63) / 64;

#if defined(__CUDACC__) || defined(__NVCC__)
        cuda_scan_kernel<<<numBlocks, threadsPerBlock>>>(
            sk0, sk1, sk2, sk3,
            chunk_size, grid_threads, cur_batches,
        ...
        u256 found_key = start_k + h_offset;
        if (h_found != 0 && found_key == expected_k) {
            passed++;
        } else {
            failed++;
            std::cerr << "[ERROR] Puzzle #" << sp.id << " CUDA verify failed\n";
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (is_verify_mode) {
        int exit_code = run_gpu_verify(verify_puzzle_id, target_device_id);
        return exit_code;
    }
